...
export function TechChip({ name, className }: TechChipProps) {
  ...
  return (
    <span
      className={cn(
        "group inline-flex items-center gap-2.5 rounded-xl border border-border bg-surface px-3.5 py-2.5 text-sm leading-none text-foreground/90 transition-[border-color,color,transform] duration-200 hover:-translate-y-0.5 hover:border-accent hover:text-foreground",
        className,
      )}
    >
      <Icon
